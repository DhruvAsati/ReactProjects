...
const onRestaurantFilter = (data) => {
    if (
        (
            data.name.toLowerCase().includes(searchKey.toLowerCase().trim()) ||
            data.address.toLowerCase().includes(searchKey.toLocaleLowerCase().trim()) ||
            data["address line 2"].toLowerCase().includes(searchKey.toLocaleLowerCase().trim()))
        && Number(data.rating) >= Number(rating)) {
        return true
    } else {
        return false
    }
  ...
  return (
    <>
      ...
      <div className="SearchRes">
            <input type="search" onChange={onSearchBoxChange}  placeholder='Search your favourite hotel'/>
             <input type="number" onChange={onRatingBoxChange} placeholder='Rating' />
        </div>
        {
          data.filter(onRestaurantFilter).map((restaurantData, index) => (
              <Card key = {restaurantData._id}{...restaurantData}/>
          ))
