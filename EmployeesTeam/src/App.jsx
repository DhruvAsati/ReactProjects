...
const reducer = (state, action) => {
  switch (action.type) {
    case "ADD_TO_TEAM":
      if (state.team.find((member) => member.id === action.payload.id)) {
        return state;
      }
      return {
        ...state,
    ...
    case "REMOVE_FROM_TEAM":
      return {
        ...state,
        team: state.team.filter((member) => member.id !== action.payload.id),
      };

    case "SORT_TEAM_BY_AGE":
      return {
        ...state,
...
function App() {
  ...
  const calculateAverageAge = () => {
    ...
    const totalAge = state.team.reduce((sum, member) => sum + member.age, 0);
    return (totalAge / state.team.length).toFixed(2);
  };

  return (
    <div style={{ display: "flex", justifyContent: "space-around", padding: "20px" }}>
      <div style={{ border: "2px solid black", padding: "20px", width: "300px" ,height: "600px", overflow: "scroll"}}>
        <h2>Employees</h2>
        {state.employees.map((employee) => (
          ...
          >
            ...
            <button
              onClick={() => dispatch({ type: "ADD_TO_TEAM", payload: employee })}
              style={{ backgroundColor: "blue", color: "white", border: "none", padding: "5px" }}
            >
              Add
            </button>
          </div>
      ...
      <div style={{ border: "2px solid black", padding: "20px", width: "300px" }}>
        <h2>TEAM</h2>
        <button
          onClick={() => dispatch({ type: "SORT_TEAM_BY_AGE" })}
          style={{
        ...
        {state.team.map((member) => (
          ...
          >
            ...
            <button
              onClick={() => dispatch({ type: "REMOVE_FROM_TEAM", payload: member })}
              style={{ backgroundColor: "blue", color: "white", border: "none", padding: "5px" }}
            >
              Remove
            </button>
        ...
        <div style={{ marginTop: "20px", backgroundColor: "#c0c0c0", padding: "10px" }}>
